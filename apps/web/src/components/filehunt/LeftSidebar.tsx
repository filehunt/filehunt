...
export function LeftSidebar() {
  ...
  const collections: CollectionInfo[] = [
    ...
    { name: 'Images', count: 423, icon: Image, type: 'media' },
  ...
  const getCollectionIconColor = (type: CollectionInfo['type'], name: string) => {
    switch (type) {
      case 'tag':
        return name === 'Tagged' ? 'text-blue-400' : 'text-muted-foreground';
      case 'filter':
        return name === 'Trash' ? 'text-red-400' : 'text-muted-foreground';
      case 'status':
  ...
  return (
    <div className="w-60 flex flex-col h-full" style={{ backgroundColor: 'transparent' }}>
      {/* Assets Section */}
          ...
          {expandedSections.assets && (
            <div className="space-y-1">
              {assetFolders.map((folder, index) => (
                <div key={index}>
                  <div className={cn(
                    "flex items-center justify-between py-2 px-2 rounded hover:bg-accent cursor-pointer group",
                    folder.name === 'Key Scenes' ? 'bg-accent' : ''
                  )}>
            ...
            <div className="space-y-1">
              {collections.map((collection, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex items-center justify-between py-2 px-2 rounded hover:bg-accent cursor-pointer group"
                  >
                    <div className="flex items-center space-x-2 flex-1 min-w-0">
                      <IconComponent className={cn("w-4 h-4 flex-shrink-0", getCollectionIconColor(collection.type, collection.name))} />
              ...
              <button className="flex items-center space-x-2 py-2 px-2 w-full text-left rounded hover:bg-accent group">
                ...
                <span className="text-xs text-muted-foreground group-hover:text-foreground">New Collection</span>
              </button>
            </div>
          )}
        </div>
        ...
        <div className="mb-6">
          ...
          {expandedSections.tags && (
            <div className="space-y-1">
              ...
              <button className="flex items-center space-x-2 py-1.5 px-2 w-full text-left rounded hover:bg-accent group">
                <Plus className="w-3 h-3 text-muted-foreground" />
                <span className="text-xs text-muted-foreground group-hover:text-foreground">Add Tag</span>
              </button>
            </div>
          )}
        </div>
          ...
          {expandedSections.recent && (
            <div className="space-y-2">
              {recentActivity.map((activity) => (
                <div
                  key={activity.id}
                  className="py-2 px-2 rounded hover:bg-accent cursor-pointer group"
                >
